...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2, 1):
            for j in range(cell[1] - 1, cell[1] + 2, 1):                
                ...
                if self.moves_made.issuperset({(i, j)}):
                    continue
        ...
        new_sentences = []
        for sentence in self.knowledge:
            intersecting_mines = len(sentence.get_cells().intersection(self.mines))
            count = sentence.get_count() - intersecting_mines
            cells = sentence.get_cells().difference(self.safes).difference(self.mines)
    ...
    def make_safe_move(self):
        ...
        for safe in self.safes:
            if not self.moves_made.issuperset({safe}):
                return safe
    ...
    def make_random_move(self):
        ...
        moves_not_made = moves_not_made.difference(self.moves_made)
        moves_not_made = moves_not_made.difference(self.mines)

        if len(moves_not_made) == 0:
            return None
        
        random_number = random.randrange(len(moves_not_made))

        count = 0

        for move_not_made in moves_not_made:
            if count == random_number:
                return move_not_made
            else:
                count += 1